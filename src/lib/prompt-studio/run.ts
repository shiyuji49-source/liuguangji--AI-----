...
export async function buildShotlist(opts: {
  ...
  const userText = [
    ...
    opts.directorStyle && opts.directorStyle !== "标准"
      ? `【导演风格（用户选定，整集一锁到底）】本集按 skill 风格预设库中的「${opts.directorStyle}」执行：运镜倾向、景别分布、构图语言、节奏、固定/运动比例全部按该预设。⚠️风格要**融进每镜的画面/景别/运镜描述本身**——summary 直接写这一镜具体发生什么（画面、动作、构图），风格体现在你给的景别/运镜/构图选择上；**禁止把风格名当 summary 开头、禁止写"${opts.directorStyle}风格执行：…""${opts.directorStyle}，如何如何"这类标签式前缀**，读起来就是一句自然的分镜描述。预设与通用法则冲突时预设优先，但轴线/视线/反应镜头原则不可破。`
      : ``,
    `【硬要求（务必做到，否则就是 low）】`,
